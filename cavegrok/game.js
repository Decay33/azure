...
window.addEventListener('keydown', (e) => {
  if (e.code === 'Space') {
    ...
    keys.space = true;
    // Play thrust sound
    if (window.soundSystem && !player.crashed && !player.landed) {
      window.soundSystem.playThrust();
    }
  ...
  obstacles.forEach(ob => {
    ...
    if (ob.type === "hill") {
      if (firingStarted && Date.now() >= ob.nextShot) {
        ...
        if (window.soundSystem) {
          window.soundSystem.playEnemyShoot();
        }
        ob.nextShot = Date.now() + 5000;
      }
    }
    ...
    if (isColliding(playerRect, platformRect)) {
      ...
      if (prev_bottom <= platform.y && 
        ...
        if (player.vy > crash_vertical_speed || Math.abs(player.vx) > crash_horizontal_speed) {
          ...
          // Play landing sound
          if (window.soundSystem) {
            window.soundSystem.playLanding();
          }
...
function renderWalls() {
  if (visualEffects) {
    // Use enhanced 3D wall rendering
    walls.forEach(wall => {
      visualEffects.draw3DWall(wall);
    });
  } else {
...
function renderPlatforms() {
  if (visualEffects) {
    // Use enhanced 3D platform rendering
    platforms.forEach(platform => {
      visualEffects.draw3DPlatform(platform);
    });
  } else {
    // Fallback to original rendering
    platforms.forEach(platform => {
...
function renderPlayer() {
    if (visualEffects) {
        // Use enhanced 3D ship rendering
        let tilt = player.vx * tilt_sensitivity;
        tilt = Math.max(-max_tilt, Math.min(max_tilt, tilt));
        
        // Add reduced thrust particles (was 3, now 1-2)
        if (keys.space && !player.crashed && !player.landed) {
            if (Math.random() < 0.7) { // Only 70% chance to spawn particle
                visualEffects.addParticle(
        ...
        visualEffects.draw3DShip(player.x, player.y, currentShip, tilt, 1);
        return; // Exit early when using 3D effects
    }
    
    // Fallback to original 2D rendering with new ship designs
    ctx.save();
    ...
    // Draw reduced thrust flame
    if (keys.space && !player.crashed && !player.landed) {
        let flameHeight = 8 + Math.random() * 3; // Reduced from 15+5 to 8+3
        let gradient = ctx.createLinearGradient(0, player.height/2, 0, player.height/2 + flameHeight);
        gradient.addColorStop(0, 'rgba(255, 255, 0, 0.8)'); // Reduced opacity
        gradient.addColorStop(0.6, 'rgba(255, 69, 0, 0.6)');
        gradient.addColorStop(1, 'rgba(255, 69, 0, 0)');
        
        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.moveTo(-player.width/6, player.height/2); // Narrower flame
    ...
    // Draw ship based on type with completely new designs
    switch(currentShip) {
        case "hotpink":
            ...
            ctx.lineTo(player.width/5, -player.height/6);
        ...
        case "purple":
            ...
            ctx.beginPath();
            ctx.arc(-player.width/4, 0, player.width/8, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(player.width/4, 0, player.width/8, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.arc(0, -player.height/6, 2, 0, Math.PI * 2);
            ctx.stroke();
            ctx.beginPath();
            ctx.arc(0, player.height/6, 2, 0, Math.PI * 2);
            ctx.stroke();
        ...
        case "golden":
            ...
            ctx.beginPath();
            ctx.moveTo(0, -player.height/2);
            ctx.lineTo(player.width/2.5, -player.height/5);
            ctx.lineTo(player.width/2, player.height/6);
            ...
            ctx.lineTo(-player.width/6, -player.height/1.5);
            ctx.lineTo(0, -player.height/1.3);
            ctx.lineTo(player.width/6, -player.height/1.5);
            ctx.lineTo(player.width/4, -player.height/2);
            ...
            ctx.beginPath();
            ctx.arc(-player.width/6, -player.height/3, 2, 0, Math.PI * 2);
            ctx.fill();
            ctx.beginPath();
            ctx.arc(player.width/6, -player.height/3, 2, 0, Math.PI * 2);
            ctx.fill();
        ...
        default:
            ...
            // Main body - angular military
            ctx.fillStyle = '#CCCCCC';
            ctx.beginPath();
            ctx.moveTo(0, -player.height/2);
            ctx.lineTo(player.width/3, -player.height/6);
            ...
            ctx.beginPath();
            ctx.moveTo(0, -player.height/3);
            ctx.lineTo(player.width/5, -player.height/8);
            ctx.lineTo(player.width/6, player.height/8);
            ctx.lineTo(-player.width/6, player.height/8);
            ctx.lineTo(-player.width/5, -player.height/8);
            ctx.closePath();
            ctx.fill();
            ...
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(-player.width/4, -player.height/6);
            ctx.lineTo(player.width/4, -player.height/6);
            ctx.moveTo(-player.width/5, player.height/6);
            ctx.lineTo(player.width/5, player.height/6);
            ctx.stroke();
    }
...
function renderObstacles() {
  if (visualEffects) {
    // Use enhanced 3D enemy ship rendering
    obstacles.forEach(ob => {
      visualEffects.draw3DEnemyShip(ob.x, ob.y, ob.width, ob.height, ob.type);
    });
  } else {
    // Fallback to original rendering
    obstacles.forEach(ob => {
...
function renderGems() {
  if (visualEffects) {
    // Use enhanced 3D gem rendering
    gems.forEach(gem => {
      visualEffects.draw3DGem(gem);
    });
  } else {
    // Fallback to original rendering
    gems.forEach(gem => {
...
function drawEngineGlow() {
  ...
  let flameHeight = 4 + glowIntensity * 2; // Reduced from 8+5 to 4+2
...
function render() {
  ...
  // Use enhanced background if visual effects are available
  if (visualEffects) {
    visualEffects.renderEnhancedBackground();
    visualEffects.updateParticles();
  } else {
    renderBackground();
  }
...
window.addEventListener('load', function() {
    ...
    if (window.VisualEffects) {
        visualEffects = new window.VisualEffects(ctx, canvas);
